...
class Game:
    ...
    def move(self, player, direction):
        if game.field.can_move(self.position, direction):
            self.position = get_new_pos(self.position, direction)
            game.field[self.position].arrive(game, self)
...
class Field:
    ...
    def can_move(self, position, direction, silent=False):
...
class SmartBot(Bot):
        ...
        while len(queue) != 0:
            ...
            #         closest = {'node': cur_node, 'distance': distance}
                ...
                for direction in shuffle(DIRECTIONS.keys()):
                    ...
                    if (self.field.can_move(cur_node, direction, True) and
    ...
    def turn(self, game):
        ...
        direction = self.route.pop(0)
        game.exec_correct_command(direction, self)
        if old_pos == self.position:
            self.route = []
            if DIRECTIONS[direction][1]:
                new_cord = old_pos[1] + min(DIRECTIONS[direction][1], 0)
                self.field.vwalls[old_pos[0]][new_cord] = True
            else:
                new_cord = old_pos[0] + min(DIRECTIONS[direction][0], 0)
                self.field.hwalls[new_cord][old_pos[1]] = True

    def try_to_exit(self, game):
        direction = self.potential_exits[self.position].pop()
        game.exec_correct_command(direction, self)
...
def main():
    ...
    for i in range(games_number):
        ...
        if isinstance(game.game(), SmartBot):
            smart_wins += 1
